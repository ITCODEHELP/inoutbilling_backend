...
### Get Data for Duplicating Export Invoice
```http
GET /api/export-invoice/:id/duplicate
Authorization: Bearer <token>
```
**Response**
Returns the Export Invoice data with system fields (ID, status, timestamps, user ID) and document-specific fields (invoice number) removed, ready to prefill a new create form.

### Cancel Export Invoice
```http
POST /api/export-invoice/:id/cancel
Authorization: Bearer <token>
```
**Response**
...
```http
POST /api/export-invoice/:id/restore
Authorization: Bearer <token>
```
**Response**
Updates status from `Cancelled` to `Active`.

### Download Export Invoice PDF
```http
GET /api/export-invoice/:id/download-pdf?original=true&duplicate=true&transport=true&office=true
Authorization: Bearer <token>
```
**Query Parameters**
- `original`, `duplicate`, `transport`, `office`: Set to `true` to include specific copies in the PDF.
- `:id` can be a single ID or a comma-separated list of IDs for merged PDF.

### Share Export Invoice via Email
```http
POST /api/export-invoice/:id/share-email
Authorization: Bearer <token>
Content-Type: application/json
```
**Request Body**
...
```
*Note: If email is not provided, it defaults to the customer's registered email.*

### Share Export Invoice via WhatsApp
```http
POST /api/export-invoice/:id/share-whatsapp
Authorization: Bearer <token>
Content-Type: application/json
```
**Request Body**
...
```
**Response**
Returns a `deepLink` to open WhatsApp with a pre-filled message and public view link.

### Generate Public Link
```http
GET /api/export-invoice/:id/public-link?original=true
Authorization: Bearer <token>
```
**Response**
Returns a secure, non-authenticated link to view the PDF.

### Public View
```http
GET /api/export-invoice/view-public/:id/:token
```
*Note: This endpoint is public and does not require a Bearer token. It uses a secure HMAC token generated by the server.*

### Attachments

#### Attach Files
```http
POST /api/export-invoice/:id/attach-file
Authorization: Bearer <token>
Content-Type: multipart/form-data
```
**Form Data**
- `attachments`: File (Accepts multiple files, max 10, 10MB each)

#### Get Attachments
```http
GET /api/export-invoice/:id/attachments
Authorization: Bearer <token>
```

#### Update/Replace Attachment
```http
PUT /api/export-invoice/:id/attachment/:attachmentId
Authorization: Bearer <token>
Content-Type: multipart/form-data
```
**Form Data**
- `attachment`: File (Single file replacement)

#### Delete Attachment
```http
DELETE /api/export-invoice/:id/attachment/:attachmentId
Authorization: Bearer <token>
```
