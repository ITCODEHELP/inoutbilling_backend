 ...
 */
const generatePurchaseInvoicePDF = (invoices, user, options = { original: true }) => {
    return new Promise((resolve, reject) => {
        ...
        invoiceList.forEach((invoice) => {
            copies.forEach((copyType) => {
                ...
                doc.fillColor(blackColor).fontSize(7).text(getCopyLabel(copyType), startX, headerBottom + 7, { align: 'right', width: totalWidth - 5 });
                ...
                const drawSummaryRow = (label, value, isBold = false, isTotal = false, hasBg = false) => {
                    ...
                    if (hasBg) {
                        doc.rect(colSplitX, rightCurY, rightColWidth, sumRowH).fill(lightBlueBg).stroke(borderColor);
                    } else {
                        doc.rect(colSplitX, rightCurY, rightColWidth, sumRowH).stroke(borderColor);
                    }
                    if (isTotal) {
